(function() {
  ...
  randomBytes = wiki.randomBytes = function(n) {
    return ((function() {
      var _i, _results;
      _results = [];
  ...
  asSlug = wiki.asSlug = function(name) {
    return name.replace(/\s/g, '-').replace(/[^A-Za-z0-9-]/g, '').toLowerCase();
  };

  pageToJson = wiki.pageToJson = function(element) {
    return {
  ...
  pagesInDom = wiki.pagesInDom = function() {
    return $('.page').dataDash('slug');
  };

  urlPages = wiki.urlPages = function() {
    var i;
    return ((function() {
  ...
  locsInDom = wiki.locsInDom = function() {
    return $('.page').dataDash('site');
  };

  urlLocs = wiki.urlLocs = function() {
  ...
  createPage = wiki.createPage = function(slug, site) {
  ...
  pushToLocal = wiki.pushToLocal = function(pageElement, action) {
    var page;
  ...
  pushToServer = wiki.pushToServer = function(pageElement, action) {
    return $.ajax({
      type: 'PUT',
  ...
  formatTime = wiki.formatTime = function(time) {
    var am, d, h, mi, mo;
  ...
  refresh = wiki.refresh = function() {
    ...
    if ($(pageElement).dataDash('server-generated')[0] === 'true') {
      ...
      if (wiki.useLocalStorage() && (json = localStorage[pageElement.dataDash('slug')[0]])) {
